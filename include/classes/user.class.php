...
class User extends Base {
    ...
    public function updateAccount($userID, $address, $threshold, $donate, $email, $timezone, $is_anonymous, $strToken)
    {
        ...
        if (!empty($address)) {
            if ($address != $this->coinAddress->getCoinAddress($userID) && $this->coinAddress->existsCoinAddress($address)) {
      ...
      GROUP BY username");
        ...
        public function getUserData($userID) {
            ...
            $stmt = $this->mysqli->prepare("
      SELECT
      a.id , a.username, a.pin, a.api_key, a.is_admin, a.is_anonymous, a.email, a.timezone, a.no_fees,
      ...
      WHERE a.id = ? LIMIT 0,1");
            if ($this->checkStmt($stmt) && $stmt->bind_param('si', $this->currency, $userID) && $stmt->execute() && $result = $stmt->get_result()) {
        ...
        public function register($username, $coinaddress, $password1, $password2, $pin, $email1='', $email2='', $tac='', $strToken='') {
            ...
            if (!is_null($coinaddress)) {
                if ($this->coinAddress->existsCoinAddress($coinaddress)) {
        ...
        ");
            ...
            if ($this->checkStmt($stmt) && $stmt->bind_param('sssissi', $username_clean, $password_hash, $email1, $signup_time, $pin_hash, $apikey_hash, $is_locked) && $stmt->execute()) {
                ...
                if (!is_null($coinaddress)) $this->coinAddress->add($new_account_id, $coinaddress);
