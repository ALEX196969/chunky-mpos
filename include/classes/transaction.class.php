...
class Transaction extends Base {
   ...
   * @return bool
   **/
  public function addTransaction($account_id, $amount, $type='Credit', $block_id=NULL, $coin_address=NULL, $txid=NULL, $convertible=NULL, $database=NULL) {
  ...
  public function setArchived($account_id, $txid) {
    ...
    $stmt = $this->mysqli->prepare("
      ...
      AND (
        ...
        OR ( t.account_id = ? AND t.id <= ? AND b.confirmations = -1 AND t.type != 'Convertible')
 ...
 public function getConvertibleQueue($limit=250) {
  ...
  public function getAPQueue($limit=250) {
    ...
    $stmt = $this->mysqli->prepare("
      ...
      LEFT JOIN " . $this->user->getTableName() . " AS a
      ON t.account_id = a.id
      LEFT JOIN " . $this->coin_address->getTableName() . " AS c
